...
class ServerConnection:
    def __init__(self):
        ...
        self._channel = grpc.secure_channel(f"[::]:{port}", grpc.local_channel_credentials())
...
class JoinedServerConnection(ServerConnection):
    def __init__(self):
        ...
        response = self.connection.send(dm_env_rpc_pb2.JoinWorldRequest(world_name=self.world_name))
...
class CatchDmEnvTest(test_utils.EnvironmentTestMixin, absltest.TestCase):
    def setUp(self):
        ...
        self._dm_env = dm_env_adaptor.DmEnvAdaptor(self._connection, self._server_connection.specs)
        super().setUp()
...
class CatchTestSettings(absltest.TestCase):
    ...
    def tearDown(self):
        try:
            if self._world_name:
                ...
                self._connection.send(dm_env_rpc_pb2.DestroyWorldRequest(world_name=self._world_name))
        finally:
            self._server_connection.close()
        super().tearDown()
    ...
    def test_reset_world_seed_setting(self):
        self._world_name = self._connection.send(
            dm_env_rpc_pb2.CreateWorldRequest(settings={"seed": tensor_utils.pack_tensor(1234)})
        ).world_name
        self._connection.send(dm_env_rpc_pb2.JoinWorldRequest(world_name=self._world_name))

        step_response = self._connection.send(dm_env_rpc_pb2.StepRequest())
        self._connection.send(
            dm_env_rpc_pb2.ResetWorldRequest(world_name=self._world_name, settings={"seed": tensor_utils.pack_tensor(1234)})
        )
        self.assertEqual(step_response, self._connection.send(dm_env_rpc_pb2.StepRequest()))
    ...
    def test_reset_seed_setting(self):
        ...
        step_response = self._connection.send(dm_env_rpc_pb2.StepRequest())
        self._connection.send(dm_env_rpc_pb2.ResetRequest(settings={"seed": tensor_utils.pack_tensor(1234)}))
        self.assertEqual(step_response, self._connection.send(dm_env_rpc_pb2.StepRequest()))
...
class CatchTest(absltest.TestCase):
    ...
    def test_cannot_join_world_with_wrong_name(self):
        with self.assertRaises(error.DmEnvRpcError):
            self._connection.send(dm_env_rpc_pb2.JoinWorldRequest(world_name="wrong_name"))
    ...
    def test_cannot_destroy_world_with_wrong_name(self):
        with self.assertRaises(error.DmEnvRpcError):
            self._connection.send(dm_env_rpc_pb2.DestroyWorldRequest(world_name="wrong_name"))
