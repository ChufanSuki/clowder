...
class CatchGame:
    ...
    def draw_board(self):
        ...
        board[self._ball_y, self._ball_x] = 1.0
        board[self._paddle_y, self._paddle_x] = 1.0
        return board
    ...
    def reward(self):
        ...
        if self.has_terminated():
            return 1.0 if self._paddle_x == self._ball_x else -1.0
        else:
            return 0
...
def _check_message_type(env, is_joined, message_type):
    ...
    if not env:
        if message_type not in ["create_world", "leave_world"]:
            raise RuntimeError(f"Cannot {message_type} when no world exists.")
    else:
        ...
        if is_joined:
            ...
            if message_type == "reset_world":
                ...
                raise RuntimeError(f"Cannot {message_type} when world not joined.")


def _observation_spec():
    """Returns the observation spec."""
    return {
        1: dm_env_rpc_pb2.TensorSpec(name=_OBSERVATION_BOARD, shape=[_NUM_ROWS, _NUM_COLUMNS], dtype=dm_env_rpc_pb2.FLOAT),
        2: dm_env_rpc_pb2.TensorSpec(name=_OBSERVATION_REWARD, dtype=dm_env_rpc_pb2.FLOAT),
    }


def _action_spec():
    """Returns the action spec."""
    paddle_action_spec = dm_env_rpc_pb2.TensorSpec(dtype=dm_env_rpc_pb2.INT8, name=_ACTION_PADDLE)
    tensor_spec_utils.set_bounds(paddle_action_spec, minimum=np.min(_VALID_ACTIONS), maximum=np.max(_VALID_ACTIONS))
...
def _validate_settings(settings, valid_settings):
    ...
    unrecognized_settings = [setting for setting in settings if setting not in valid_settings]

    if unrecognized_settings:
        raise ValueError("Unrecognized settings provided! Invalid settings:" f" {unrecognized_settings}")


class CatchGameFactory:
    ...
    def Process(self, request_iterator, context):
        ...
        for request in request_iterator:
            ...
            try:
                ...
                if message_type == "create_world":
                    _validate_settings(request.create_world.settings, valid_settings=_VALID_CREATE_AND_RESET_SETTINGS)
                    ...
                    packed_observations = observation_manager.pack(
                        {_OBSERVATION_BOARD: env.draw_board(), _OBSERVATION_REWARD: env.reward()}
                    )
                    ...
                    _validate_settings(request.reset.settings, valid_settings=_VALID_CREATE_AND_RESET_SETTINGS)
                    ...
                    _validate_settings(request.reset_world.settings, valid_settings=_VALID_CREATE_AND_RESET_SETTINGS)
                    ...
                    if internal_request.world_name != _WORLD_NAME:
                        raise RuntimeError(
                            'Tried to destroy world "{}" but we only support world "{}"'.format(
