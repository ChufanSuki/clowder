...
Minimal implementations of fake Clowder components which can be instantiated in
...
class MockVariableSource(VariableSource):
    ...
    def get_variables(self, names: Sequence[str]) -> List[specs.NestedArray]:
...
class _BaseDiscreteEnvironment(Environment):
    ...
    def __init__(self,
        ...
        if discount_spec is None:
            discount_spec = BoundedArray((), np.float32, 0.0, 1.0)
...
class MockContinuousEnvironment(Environment):
    ...
    """
        ...
        discounts = BoundedArray((), reward_dtype, 0.0, 1.0)

        if bounded:
            actions = BoundedArray(action_shape, dtype, -1.0, 1.0)
        else:
